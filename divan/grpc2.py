...
class AssistantServiceClient:
    ...
    def _assist(self, recorder, play, deadline):
        ...
        for response in self._assistant.Assist(self._requests(recorder), deadline):
            ...
            if response.speech_results:
                ...
                logger.info('You said muhahaha bitches this time: "%s".',
                           result) 
                result = result.lower()
                if ('candy' in result):
                    logger.debug('SHIIIIIIIIT YO BOOIII')
                    return False
    ...
    def _assist_2(self, recorder, play, deadline):
        continue_conversation = False
        logger.debug("AREEEEEEEEEEEEEEEEEE WE HEEEEEEEEEEEEEEEEEEEEEEEEEEEEERE")
        for response in self._assistant.Assist(self._requests(recorder), deadline):
            if response.event_type == END_OF_UTTERANCE:
                logger.info('End of audio request detected.')
                recorder.done()
            ...
            if response.speech_results:
                ...
                logger.info('You said assist2: "%s".',
                            result)
                result = result.lower()
                flag = ('starbucks' in result.lower())
                logger.debug("Heyyyyyyy boiiiiiiiiiiiiiiii", flag);
                if flag:
                    logger.info("Found dis shit")
                    return False
    ...
    def conversation(self, deadline=DEFAULT_GRPC_DEADLINE):
        ...
        while keep_talking:
            ...
            with Recorder() as recorder, BytesPlayer() as player:
                ...
                try:
                    logger.debug("FUCKIIIIIIIINNNNNNNNNNNNNNNN GAAAAAAAAAAAAAWD")
    ...
    def conversation_2(self, deadline=DEFAULT_GRPC_DEADLINE):
        """
        Starts a conversation with the Google Assistant.
        ...
        while keep_talking:
            ...
            with Recorder() as recorder, BytesPlayer() as player:
                ...
                def wrapped_play(data):
                  ...
                  #  self._assist(recorder,wrapped_play,deadline)
                try:
    ...
    def conversation2(self, deadline=DEFAULT_GRPC_DEADLINE):
        ...
        while keep_talking:
            ...
            if playing:
                self._playing_stopped()
        logger.debug("Can we finally start recording shit here")
